...
title: "Wine: LiquidAssets: Price regression:: LiquidAssets_Wine_2016"
author: "bdanalytics"
...
Data: 
Source: 
    Training:   https://d37djvu3ytnwxt.cloudfront.net/asset-v1:MITx+15.071x_3+1T2016+type@asset+block/wine.csv  
    New:        https://d37djvu3ytnwxt.cloudfront.net/asset-v1:MITx+15.071x_3+1T2016+type@asset+block/wine_test.csv  
Time period: 
...
glbObsTrnFile <- list(url = "https://d37djvu3ytnwxt.cloudfront.net/asset-v1:MITx+15.071x_3+1T2016+type@asset+block/wine.csv" 
...
glbObsNewFile <- list(url = "https://d37djvu3ytnwxt.cloudfront.net/asset-v1:MITx+15.071x_3+1T2016+type@asset+block/wine_test.csv") 
...
glbFeatsId <- "Year" # choose from c(NULL : default, "<id_feat>") 
...
glbOut <- list(pfx = "LiquidAssets_Wine_2016_")
...
#load("LiquidAssets_Wine_2016_extract.features.end.RData", verbose = TRUE)
...
if (!is.null(glbObsNewFile)) {
    ...
    if (is.null(glbObsTrnFile$splitSpecs) ||
        ...
        glbObsNew <- do.call("subset", 
            list(glbObsTrn, parse(text = glbObsTrnFile$splitSpecs$condition)))
        glbObsTrn <- do.call("subset", 
            list(glbObsTrn, parse(text = paste0("!(", 
                                                  glbObsTrnFile$splitSpecs$condition,
                                                  ")"))))
...
myderiveFeatures <- function(obsDf, deriveFtrs) {
    for (new_feat in names(deriveFtrs)) {
        if (new_feat %in% names(obsDf)) {
            warning(new_feat, " already present in glbObsAll, skipping ...")
            next
        }
        
        args_lst <- NULL 
        for (arg in deriveFtrs[[new_feat]]$args)
            if (!(arg %in% names(obsDf))) {
                # not all args avl. yet
                warning("arg ", arg, " not available yet...")
                args_lst <- NULL
                break
            } else args_lst[[arg]] <- obsDf[, arg]
        
        if (!is.null(args_lst)) {    
            print(sprintf("Creating new feature: %s...", new_feat))
            obsDf[, new_feat] <- do.call(deriveFtrs[[new_feat]]$mapfn, args_lst)
            rm(args_lst)
        }
    }
...
glbObsAll <- myderiveFeatures(glbObsAll, glbFeatsDerive)
...
if ((num_nas <- sum(is.na(glbObsAll[glbObsAll$.src == "Train", glb_rsp_var_raw]))) > 0) {
    ...
    myprint_df(glbObsAll[is.na(glbObsAll[glbObsAll$.src == "Train", glb_rsp_var_raw]), ])
...
if (is.null(glbObsNewFile) || 
    is.null(glbObsTrnFile$splitSpecs$method) || 
    (glbObsTrnFile$splitSpecs$method != "copy")) {
...
if (!is.null(glb_map_vars)) {
    for (feat in glb_map_vars) {
        map_df <- myimport_data(specs = list(url = glb_map_urls[[feat]]), 
                                            comment = "map_df", force_header = TRUE)
...
if (length(glbFeatsDateTime) > 0) {
    ...
    for (feat in names(glbFeatsDateTime)) {
        ...
        pltObsAll[, ".freq"] <- 1
        if (length(unique(pltObsAll[, glb_rsp_var])) <= 5)
            print(myplot_bar(pltObsAll, xcol_name = paste0(feat, ".month.fctr"), 
                             ycol_names = ".freq", colorcol_name = paste0(feat, ".wkend")) + 
                      facet_wrap(reformulate(glb_rsp_var))) else {
            pltObsAll[, paste0(glb_rsp_var, ".cut")] <- cut(pltObsAll[, glb_rsp_var], 5)
            print(myplot_bar(pltObsAll, xcol_name = paste0(feat, ".month.fctr"), 
                             ycol_names = ".freq", colorcol_name = paste0(feat, ".wkend")) + 
                      facet_wrap(reformulate(paste0(glb_rsp_var, ".cut"))))                          
        }
...
#stop(here"); glb2Sav(); all.equal(savObsAll, glbObsAll); glbObsAll <- savObsAll
        if (nrow(tmPolyDf <- myextractTimePoly(obs = glbObsAll, feat = feat)) > 0)
            glbObsAll <- cbind(glbObsAll, tmPolyDf)
        ...
        for (last in grep(paste0(feat, ".last"), names(glbObsAll), fixed = TRUE,
                          value = TRUE)) {
            if (is.numeric(glbObsAll[, glb_rsp_var]))
                print(myplot_scatter(glbObsAll, last, glb_rsp_var)) else
...
if (all(is.na(glbObsNew[, glb_rsp_var]))) {
    ...
    if (is.null(glbFeatsCategory) || (glbFeatsCategory == ".category")) {
        require(caTools)
        split <- sample.split(glbObsTrn[, glb_rsp_var_raw], 
...
#glbObsAll <- savObsAll; glbObsTrn <- savObsTrn; glbObsNew <- savObsNew  
if (glb_is_classification && glb_is_binomial)
    prob_threshold_def <- 
...
for (obsSet in c("trn", "new")) {
    ...
    obs_df <- glb_get_predictions(df = obs_df, mdl_id = glb_fin_mdl_id, 
                                    rsp_var = glb_rsp_var, 
...
if (!(glb_is_classification && !glb_is_binomial)) {
    ...
    tmpFeatsImp[, "imp.mean"] <- sapply(1:nrow(tmpFeatsImp), function(rowIx)
                                    mean(as.numeric(tmpFeatsImp[rowIx, ]), na.rm = TRUE))
    print(orderBy(as.formula("~ -imp.mean"), 
                  subset(tmpFeatsImp, imp.mean > 10)))
}
